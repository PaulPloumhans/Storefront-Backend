...
describe('api endpoint order testing', () => {
    ...
    it('CREATE route - test POST on /orders', async () => {
        let res = await request.post('/orders').send({ user_id: user_1.id });
        expect(res.status).toEqual(200);
        expect(res.body.user_id).toEqual(user_1.id);
        expect(res.body.status).toEqual('active');
        id_order_1 = res.body.id;
        res = await request.post('/orders').send({ user_id: user_1.id });
        expect(res.status).toEqual(200);
        expect(res.body.user_id).toEqual(user_1.id);
        expect(res.body.status).toEqual('active');
        id_order_2 = res.body.id;
        res = await request.post('/orders').send({ user_id: user_2.id });
        expect(res.status).toEqual(200);
    ...
    it('SHOW route - test GET on /orders/:id', async () => {
        const res = await request.get('/orders/' + id_order_1.toString());
        expect(res.status).toEqual(200);
        expect(res.body.user_id).toEqual(user_1.id);
        expect(res.body.status).toEqual('active');
    });
    ...
    it('Set an order status to complete route - test POST on /orders/complete', async () => {
        const res = await request.post('/orders/complete').send({ id: id_order_1 });
        expect(res.status).toEqual(200);
        expect(res.body.status).toEqual('complete');
    });

    // check that user_1 has only one active order (id_order_2)
    it('Current order by user route - test GET on /orders/current', async () => {
        console.log('user_id = ', user_1.id);
        const res = await request.get('/orders-current-by-userid').send({ user_id: user_1.id });
        expect(res.status).toEqual(200);
    ...
    it('Complete order by user route - test GET on /orders/active', async () => {
        const res = await request.get('/orders-complete-by-userid').send({ user_id: user_1.id });
        expect(res.status).toEqual(200);
    ...
    it('DELETE route - test DELETE on /orders', async () => {
        let res = await request.delete('/orders').send({ id: id_order_1 });
        expect(res.status).toEqual(200);
        expect(res.body.id).toEqual(id_order_1);
        res = await request.delete('/orders').send({ id: id_order_2 });
        expect(res.status).toEqual(200);
        expect(res.body.id).toEqual(id_order_2);
        res = await request.delete('/orders').send({ id: id_order_3 });
        expect(res.status).toEqual(200);
